...
class JointJetLens:
    ...
    def __init__(self, lm: LM, layers: list[int], order: int):
        centers = [lm.residual_stream(layers[0])] + [
            (lambda a, b: lambda z: lm.residual_stream(b)(z) - lm.residual_stream(a)(z))(layers[i - 1], layers[i])
    ...
    def jet_logits(self, z: Tensor) -> tuple[Tensor, list[Tensor]]:
        ...
        exps, _ = self._jet_out.expansions_and_remainder_with_unembedding(z)
        if self._log_weights is None:
            w = torch.full((self._n,), 1.0 / self._n)
    ...
    def _loss(self, z: Tensor) -> Tensor:
        ...
        exps, _ = self._jet_out.expansions_and_remainder(z)  # hidden (ln) space
        combo = sum(
    ...
    def optimize_weights(self, z: Tensor, lr: float = 1e-3, iters: int = 100) -> list[float]:
        ...
        seq_len = z.shape[-1]
        self._log_weights = nn.Parameter(torch.zeros(self._n, seq_len))
