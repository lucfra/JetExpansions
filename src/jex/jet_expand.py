...
class JetExpansionOut:
    ...
    def expansions_and_remainder(
        self, z: Tensor, with_unembedding: bool = False
    ) -> tuple[list[Tensor], Tensor]:
        """Compute expansions and remainder in one pass, reusing term evaluations."""
        assert len(self.terms) > 0, "No expansion terms — provide at least one center."
        exps = [term(z) for term in self.terms]
...
def jet_expand(
    ...
    assert weights.shape == (n,), (
        f"Weight should be a vector of length n: {n}; got\n{weights}"
    )
    if not isinstance(variate, CachedF):
        variate = CachedF(variate)
    terms = [
        JetExpandedTerm(f, centers[i], variate, order, weights[i]) for i in range(n)
    ]
...
def jet_expand_lm(
    ...
    assert 1 <= layer <= lm.depth + 1, (
        f"layer must be in [1, depth+1={lm.depth + 1}], got {layer}"
    )
