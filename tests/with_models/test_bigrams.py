...
def test_eu(lm: LM, bs):
    eu_expansion = embedding_decoder(lm)
    one_grams = make_vocabulary(lm, 1, batch_size=bs)
    exps, remainder = eu_expansion.expansions_and_remainder(
        next(one_grams), with_unembedding=True
    )
...
def test_embedding_mlp_unembedding_path(gpt2: LM):
    ...
    jet_out = embedding_mlp_decoder(gpt2, layer)
    exps = jet_out.expansions(tokens, with_unembedding=True)
